...
export async function GET(
  ...
  await dbConnect();
  const milestone = await MilestoneModel.findById(params.id);
  if (!milestone) {
...
export async function PATCH(
  ...
  const updated = await MilestoneModel.findByIdAndUpdate(params.id, parsed.data, { new: true });
  if (!updated) {
    return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });
  }
...
export async function DELETE(
  ...
  await dbConnect();
  const deleted = await MilestoneModel.findByIdAndDelete(params.id);
  if (!deleted) {
    return NextResponse.json({ success: false, error: "Not found" }, { status: 404 });
  }
